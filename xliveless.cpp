...
extern CHAR g_szUserName[4][XUSER_NAME_SIZE];
extern UINT g_online;
extern CHAR g_profileDirectory[];
extern CHAR g_password[XUSER_PASSWORD_SIZE];
...
BOOL XNotifyGetNext_Compat(HANDLE hNotification, DWORD dwMsgFilter, PDWORD pdwId, PULONG_PTR pParam)
{
	...
	if (print < print_limit)
	{
		...
		TRACE("XNotifyGetNext  (hNotification = %X, dwMsgFilter = %X, pdwId = %X, pParam = %X)", hNotification, dwMsgFilter, pdwId, pParam);
...
// <summary>
// Send custom packet 'HandShake' and receive response [XSocketConnect].
// </summary>
// <info>
// [0] - XUID
// [8] - field20
...
// [17] - packet
// </info>
BOOL sendPacketHandShake(SOCKET s)
{

	BYTE handShakeResult;
	...
	size_t uLength = strlen(g_szUserName[0]);
	size_t pLength = strlen(g_password);
	size_t rLength = GAMEPACKETHEADERSIZE + lenPos + uLength + lenPos + pLength;	

	char sendBuffer[GAMEPACKETHEADERSIZE + lenPos + XUSER_NAME_SIZE + lenPos + XUSER_PASSWORD_SIZE] = { 0 };
	char recvBuf[GAMEPACKETHEADERSIZE + 1] = { 0 };

	sendBuffer[0] = xFakeXuid[0];
	sendBuffer[8] = 1;
	sendBuffer[9] = 1;
	sendBuffer[10] = 1;
	*reinterpret_cast<std::uint32_t*>(&sendBuffer[11]) = rLength - GAMEPACKETHEADERSIZE;
	sendBuffer[15] = 3;
	sendBuffer[16] = 0;

	*reinterpret_cast<std::uint32_t*>(&sendBuffer[GAMEPACKETHEADERSIZE]) = uLength;
	memcpy(&sendBuffer[GAMEPACKETHEADERSIZE + 4], g_szUserName[0], uLength);
	*reinterpret_cast<std::uint32_t*>(&sendBuffer[GAMEPACKETHEADERSIZE + 4 + uLength]) = pLength;
	memcpy(&sendBuffer[GAMEPACKETHEADERSIZE + 4 + uLength + 4], g_password, pLength);
	
	while (i < 5)
	{

		if (difftime(time(0), runTime) > 2)
		{

			send(s, sendBuffer, rLength, 0);
		...
		iRecv = recv(s, recvBuf, sizeof(recvBuf), 0);

		if (iRecv >= GAMEPACKETHEADERSIZE)
		{

			uint32_t packetLength = *reinterpret_cast<std::uint32_t*>(&recvBuf[11]);
			BYTE packetType = *reinterpret_cast<BYTE*>(&recvBuf[15]);
			
			if (iRecv - GAMEPACKETHEADERSIZE >= packetLength)
			{

				if (packetType == static_cast<BYTE>(4))
				{

					handShakeResult = *reinterpret_cast<BYTE*>(&recvBuf[GAMEPACKETHEADERSIZE]);

					TRACE("XSocketConnect  ( SendPacketHandShake success. BPacketType = %d, Response = %d, iRecv = %d)", packetType, handShakeResult, iRecv);

					if (handShakeResult == static_cast<BYTE>(1))
					{
...
int WINAPI XSocketSetSockOpt(SOCKET s, int level, int optname, const char *optval, int optlen)
{
    ...
    TRACE("XSocketSetSockOpt  (socket = %X, level = %d, optname = %d, optval = %s, optlen = %d)", s, level, optname, optval ? optval : "", optlen);
...
SOCKET WINAPI XSocketBind(SOCKET s, const struct sockaddr *name, int namelen)
{
	...
	TRACE("XSocketBind  (socket = %X, name = %X, namelen = %d)", s, name, namelen);
...
int WINAPI XSocketConnect(SOCKET s, const sockaddr_in* name, int namelen)
{
	...
	TRACE("XSocketConnect  ( Socket = %X, name = %X, namelen = %d) ip: %hs port: %d )", s, name, namelen, inet_ntoa(name->sin_addr), ntohs(name->sin_port));
	
	int retError = WSAGetLastError();

	if (connect(s, reinterpret_cast<sockaddr const*>(name), namelen) == SOCKET_ERROR) 
	{

		int connectError = WSAGetLastError();
		
		if (connectError == 10056 || connectError == 10035)
		{

			WSASetLastError(retError);

		} else {

			TRACE("XSocketConnect  ( Connect function failed with error: %ld )", connectError);
	...
	if (ntohs(name->sin_port) == 1500u || ntohs(name->sin_port) == 1510u)
	{

		return sendPacketHandShake(s) ? 0 : -1;
...
int WINAPI XSocketListen(SOCKET s, int backlog)
{
	...
	TRACE("XSocketListen  (socket = %X, backlog = %X)", s, backlog);
...
SOCKET WINAPI XSocketAccept(SOCKET s, struct sockaddr *addr, int *addrlen)
{
	...
	if( print < 25 )
	{
		...
		TRACE("XSocketAccept  (socket = %X, addr = %X, addrlen = %d)", s, addr, *addrlen);
...
BOOL WINAPI XSocketWSAGetOverlappedResult(SOCKET s, LPWSAOVERLAPPED lpOverlapped, LPDWORD lpcbTransfer, BOOL fWait, LPDWORD lpdwFlags)
{
	...
	TRACE("XSocketWSAGetOverlappedResult  (socket = %X, lpWSAOverlapped = %X, lpcbTransfer = %X, fWait = %d, lpdwFlags = %X)", s, lpOverlapped, lpcbTransfer, fWait, lpdwFlags);
...
int WINAPI XSocketWSASend(SOCKET s, LPWSABUF lpBuffers, DWORD dwBufferCount, LPDWORD lpNumberOfBytesSent, DWORD dwFlags, LPWSAOVERLAPPED lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
	...
	TRACE("XSocketWSASend (SOCKET s = %X, LPWSABUF lpBuffers = %X, DWORD dwBufferCount = %d, LPDWORD lpNumberOfBytesSent = %d, DWORD dwFlags = %d, LPWSAOVERLAPPED lpOverlapped = %X, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine = %X)", s, lpBuffers, dwBufferCount, * lpNumberOfBytesSent, dwFlags, lpOverlapped, lpCompletionRoutine);
...
// #33: XSocketWSAWaitForMultipleEvents
...
int WINAPI XNetRandom(BYTE * pb, DWORD cb)
{
	...
	TRACE("XNetRandom  (pb = %X, cb = %d)", pb, cb);    
...
DWORD WINAPI XNetQosListen( XNKID *pxnkid, PBYTE pb, UINT cb, DWORD dwBitsPerSec, DWORD dwFlags )
{
	...
	TRACE("XNetQosListen  (pxnkid = %X, pb = %X, cb = %d, bitsPerSec = %d, flags = %X", pxnkid, pb, cb, dwBitsPerSec, dwFlags);
...
BOOL WINAPI XCustomGetLastActionPressEx(int a1, int a2, int a3, int a4, int a5) 
{
...
BOOL WINAPI XNotifyGetNext(HANDLE hNotification, DWORD dwMsgFilter, PDWORD pdwId, PULONG_PTR pParam)
{
	...
	if((strstr(gameName, "DOW2.exe") != 0) || (strstr(gameName, "RaccoonCity.exe") != 0) || (strstr(gameName, "bsp.exe") != 0))
	{
	...
	if((g_listener[curlist].area & XNOTIFY_SYSTEM ) && dwMsgFilter == 0)
	{
	...
	if(( g_listener[curlist].area & XNOTIFY_LIVE ) && dwMsgFilter == 0)
	{
...
DWORD WINAPI XGetOverlappedExtendedError(PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XGetOverlappedExtendedError  (pOverlapped = %X) (internalLow = %X, internalHigh = %X, hEvent = %X, error = %X)", pOverlapped, pOverlapped->InternalLow, pOverlapped->InternalHigh, pOverlapped->hEvent, pOverlapped->dwExtendedError);
...
DWORD WINAPI XGetOverlappedResult(PXOVERLAPPED pOverlapped, DWORD * pResult, DWORD bWait)
{
	...
	TRACE("XGetOverlappedResult  (pOverlapped = %X, pResult = %X, bWait = %d)  (internalLow = %X, internalHigh = %X)", pOverlapped, pResult, bWait, pOverlapped->InternalLow, pOverlapped->InternalHigh);
...
int WINAPI XLiveInput(DWORD * a1)
{
	...
	if(print < 15)
	{
		...
		TRACE("XLiveInput  (a1 = %X) (00 = %X, 04 = %X, 08 = %X, 0C = %X, 10 = %X, 14 = %X, 18 = %X", a1, a1[0], a1[1], a1[2], a1[3], a1[4], a1[5], a1[6]);
...
int WINAPI XHVCreateEngine( PXHV_INIT_PARAMS pParams, PHANDLE phWorkerThread, PIXHV2ENGINE *ppEngine )
{
	...
	TRACE("XHVCreateEngine  (pParams = %X, phWorkerThread = %X, pEngine = %X)", pParams, phWorkerThread, ppEngine);
...
LONG WINAPI XLivePBufferAllocate(DWORD size, FakePBuffer **pBuffer)
{
	...
	if(print < 35)
	{
		...
		TRACE("XLivePBufferAllocate  (XEncryptedAlloc) (size = %d, pBuffer = %X)", size, pBuffer);
	...
	*pBuffer = (FakePBuffer*)HeapAlloc(hHeap,HEAP_ZERO_MEMORY,sizeof(FakePBuffer));
	...
	if(print < 35)
	{
		...
		TRACE("- buffer_new = %X, size = %d, handle = %X", *pBuffer, size, g_dwFakePData);
...
DWORD WINAPI XShowKeyboardUI(DWORD dwUserIndex, DWORD dwFlags, LPCWSTR wseDefaultText, LPCWSTR wszTitleText, LPCWSTR wszDescriptionText, LPWSTR wszResultText, DWORD cchResultText, PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XShowKeyboardUI  (dwUserIndex = %d, dwFlags = %X, wseDefaultText = %s, wszTitleText = %s, wszDescriptionText = %s, wszResultText = %X, cchResultText = %X, pOverlapped = %X)", dwUserIndex, dwFlags, wseDefaultText, wszTitleText, wszDescriptionText, wszResultText, cchResultText, pOverlapped);
...
int WINAPI XEnumerate(HANDLE hEnum, CHAR *pvBuffer, DWORD cbBuffer, PDWORD pcItemsReturned, PXOVERLAPPED pOverlapped)
{
	...
	if(print < 100)
	{
		...
		TRACE("XEnumerate  (hEnum = %X, pvBuffer = %X, cbBuffer = %X, pcItemsReturned = %X, pOverlapped = %X)", hEnum, pvBuffer, cbBuffer, pcItemsReturned, pOverlapped);
	...
	if (hEnum == g_dwOnlineFriendsList) 
	{
		...
		_XONLINE_FRIEND fr;
		strcpy(fr.szGamertag, "FireSinger");
		fr.xuid = 123;
		fr.dwFriendState = 1;
...
int WINAPI XUserGetXUID(DWORD dwUserIndex, PXUID pXuid)
{
	...
	if(print < 15)
	{
		...
		TRACE("XUserGetXUID  (userIndex = %d, pXuid = %X)", dwUserIndex, pXuid);
...
DWORD WINAPI XUserCheckPrivilege(DWORD dwUserIndex, DWORD privilegeType, PBOOL pfResult)
{
	...
	if( print < 15 )
	{
		...
		TRACE("XUserCheckPrivilege  (userIndex = %d, privilegeType = %d, pfResult = %X)", dwUserIndex, privilegeType, pfResult);
...
int WINAPI XUserGetSigninInfo(DWORD dwUserIndex, DWORD dwFlags, PXUSER_SIGNIN_INFO pSigninInfo)
{
	...
	if( pSigninInfo && g_signin[ dwUserIndex ] )
	{
		...
		if (print < 15)
		{
			...
			TRACE("XUserGetSigninInfo  (Xuid %llu signed in - Online)", pSigninInfo->xuid);
	...
	else
	{
		...
		if (print < 15)
		{
			...
			TRACE("XUserGetSigninInfo  (Xuid %llu - Could not be signed in)", pSigninInfo->xuid);
...
DWORD WINAPI XUserSetContext(DWORD dwUserIndex, DWORD dwContextId, DWORD dwContextValue)
{
	...
	TRACE("XUserSetContext  (userIndex = %d, contextId = %d, contextValue = %d)", dwUserIndex, dwContextId, dwContextValue);
...
DWORD WINAPI XUserWriteAchievements(DWORD count, PXUSER_ACHIEVEMENT pAchievement, LPVOID pOverlap )
{
	...
	TRACE("XUserWriteAchievements  (count = %x, buffer = %x, overlap = %x)", count, pAchievement, pOverlap);
...
DWORD WINAPI XUserReadStats(DWORD dwTitleId, DWORD dwNumXuids, CONST XUID *pXuids, DWORD dwNumStatsSpecs, DWORD *pSpecs, DWORD *pcbResults, DWORD *pResults, PXOVERLAPPED pOverlapped )
{
	...
	TRACE("XUserReadStats  (titleId = %X, numXuids = %d, pXuids = %X, numStatsSpecs = %d, pSpecs = %X, pcbResults = %X, pResults = %X, pOverlapped = %X", dwTitleId, dwNumXuids, pXuids, dwNumStatsSpecs, pSpecs, pcbResults, pResults, pOverlapped);
...
int WINAPI XUserSetContextEx(DWORD dwUserIndex, DWORD dwContextId, DWORD dwContextValue, PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XUserSetContextEx  (userIndex = %d, contextId = %d, contextValue = %X, pOverlapped = %X)", dwUserIndex, dwContextId, dwContextValue, pOverlapped);
...
int WINAPI XUserSetPropertyEx(DWORD dwUserIndex, DWORD dwPropertyId, DWORD cbValue, void * pvValue, PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XUserSetPropertyEx  (userIndex = %d, propertyId = %X, cbValue = %d, pvValue = %X, pOverlapped = %X)", dwUserIndex, dwPropertyId, cbValue, pvValue, pOverlapped);
...
LONG WINAPI XSessionCreate(DWORD dwFlags, DWORD dwUserIndex, DWORD dwMaxPublicSlots, DWORD dwMaxPrivateSlots, ULONGLONG *pqwSessionNonce, PXSESSION_INFO pSessionInfo, PXOVERLAPPED pOverlapped, HANDLE *phEnum)
{
	...
	TRACE("XSessionCreate  (flags = %X, userIndex = %d, maxPublicSlots = %d, maxPrivateSlots = %d, sessionNonce = %X, pSessionInfo = %X, pOverlapped = %X, handle = %X)", dwFlags, dwUserIndex, dwMaxPublicSlots, dwMaxPrivateSlots, pqwSessionNonce, pSessionInfo, pOverlapped, phEnum);
...
DWORD WINAPI XStringVerify(DWORD dwFlags, const CHAR *szLocale, DWORD dwNumStrings, const STRING_DATA *pStringData, DWORD cbResults, STRING_VERIFY_RESPONSE *pResults, PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XStringVerify  (dwFlags = %X, szLocale = %X, dwNumStrings = %d, pStringData = %X, cbresults = %d, pResults = %X, pXOverlapped = %X)", dwFlags, szLocale, dwNumStrings, pStringData, cbResults, pResults, pOverlapped);
...
DWORD WINAPI XStorageUploadFromMemory(DWORD dwUserIndex, const WCHAR *wszServerPath, DWORD dwBufferSize, const BYTE *pbBuffer, PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XStorageUploadFromMemory  (dwUserIndex = %d, wszServerPath = %s, dwBufferSize = %X, pbBuffer = %X, pXOverlapped = %X)", dwUserIndex, wszServerPath, dwBufferSize, pbBuffer, pOverlapped);
...
int WINAPI XStorageEnumerate(DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD)   
{
...
int WINAPI XStorageBuildServerPathByXuid(int a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8)
{
	...
	TRACE("XStorageBuildServerPathByXuid  (a1 = %X, a2 = %X, a3 = %X, a4 = %X, a5 = %X, a6 = %X, a7 = %X, a8 = %X", a1, a2, a3, a4, a5, a6, a7, a8);
...
DWORD WINAPI XFriendsCreateEnumerator(DWORD dwUserIndex, DWORD dwStartingIndex, DWORD dwFriendstoReturn, DWORD *pcbBuffer, HANDLE * phEnum)
{
	...
	if (pcbBuffer)
	{
		...
		*pcbBuffer = dwFriendstoReturn * sizeof(_XONLINE_FRIEND);
...
DWORD WINAPI XSessionWriteStats(DWORD, DWORD, DWORD, DWORD, DWORD, PXOVERLAPPED pOverlapped )
{
	...
	TRACE("XSessionWriteStats  (pOverlapped = %X)", pOverlapped);
...
int WINAPI XSessionStart(HANDLE hSession, DWORD dwFlags, PXOVERLAPPED pOverlapped )
{
	...
	TRACE("XSessionStart  (hSession = %X, dwFlags = %X, pOverlapped = %X", hSession, dwFlags, pOverlapped);
...
DWORD WINAPI XSessionJoinLocal(HANDLE hSession, DWORD dwUserCount, const DWORD *pdwUserIndexes, const BOOL *pfPrivateSlots, PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XSessionJoinLocal  (hSession = %X, dwUserCount = %X, pdwUserIndexes = %X, pfPrivateSlots = %X, pOverlapped = %X)", hSession, dwUserCount, pdwUserIndexes, pfPrivateSlots, pOverlapped);
...
DWORD WINAPI XSessionGetDetails(HANDLE hSession, PDWORD pcbResultsBuffer, PXSESSION_LOCAL_DETAILS pSessionDetails, PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XSessionGetDetails  (hSession = %X, pcbResultsBuffer = %X (%X), pSessionDetails = %X, pOverlapped = %X)", hSession, pcbResultsBuffer, *pcbResultsBuffer, pSessionDetails, pOverlapped);
...
DWORD WINAPI XUserReadProfileSettings (DWORD dwTitleId, DWORD dwUserIndex, DWORD dwNumSettingIds, DWORD * pdwSettingIds, DWORD * pcbResults, XUSER_READ_PROFILE_SETTING_RESULT * pResults, PXOVERLAPPED pOverlapped)
{
	
	TRACE("XUserReadProfileSettings  (TitleId = %d, UserIndex = %d, NumSettingIds = %d, pdwSettingIds = %X, pcbResults = %d, pResults = %X, pOverlapped = %X)", dwTitleId, dwUserIndex, dwNumSettingIds, pdwSettingIds, *pcbResults, pResults, pOverlapped);
	...
	if( pcbResults )
	{
		...
		if (*pcbResults == 0 )
		{
			...
			for( int lcv = 0; lcv < dwNumSettingIds; lcv++ )
			{
				...
				wsprintf( strw, L"- Settings %d: %X  (Type = %X, Size = %d", lcv+1, pdwSettingIds[lcv], settingType, settingSize);
...
DWORD WINAPI XTitleServerCreateEnumerator(LPCSTR pszServerInfo, DWORD cItem, DWORD * pcbBuffer, PHANDLE phEnum)
{
	...
	if (pcbBuffer)
	{
		...
		*pcbBuffer = sizeof(XTITLESERVER_INFO) * cItem;
...
DWORD WINAPI XUserWriteProfileSettings(DWORD dwUserIndex, DWORD dwNumSettings, const PXUSER_PROFILE_SETTING pSettings, PXOVERLAPPED pOverlapped )
{
	...
	TRACE("XUserWriteProfileSettings  (dwUserIndex = %d, dwNumSettings = %d, pSettings = %X, pOverlapped = %X)", dwUserIndex, dwNumSettings, pSettings, pOverlapped);
	...
	for( int lcv = 0; lcv < dwNumSettings; lcv++ )
	{
		...
		TRACE("- [%d] source = %d, id = %X, type = %d, size = %X, sub-id = %X, type2 = %d", lcv, pSettings[lcv].source, pSettings[lcv].dwSettingId, type, size, id, pSettings[lcv].data.type);
...
DWORD WINAPI XUserReadProfileSettingsByXuid(DWORD dwTitleId, DWORD dwUserIndexRequester, DWORD dwNumFor, const XUID *pxuidFor, DWORD dwNumSettingIds, const DWORD *pdwSettingIds, DWORD *pcbResults, PXUSER_READ_PROFILE_SETTING_RESULT pResults, PXOVERLAPPED pOverlapped)
{
...
DWORD WINAPI XStorageBuildServerPath( DWORD dwUserIndex, XSTORAGE_FACILITY StorageFacility, const void *pvStorageFacilityInfo, DWORD dwStorageFacilityInfoSize, LPCWSTR *pwszItemName, WCHAR *pwszServerPath, DWORD *pdwServerPathLength )
{
	
	TRACE("XStorageBuildServerPath  (dwUserIndex = %d, StorageFacility = %d, pvStorageFacilityInfo = %X, dwStorageFacilityInfoSize = %X, pwszItemName = %s, pwszServerPath = %X, pdwServerPathLength = %X )", dwUserIndex, StorageFacility, pvStorageFacilityInfo, dwStorageFacilityInfoSize, pwszItemName, pwszServerPath, pdwServerPathLength);
...
DWORD WINAPI XStorageDownloadToMemory( DWORD dwUserIndex, const WCHAR *wszServerPath, DWORD dwBufferSize, const BYTE *pbBuffer, DWORD cbResults, XSTORAGE_DOWNLOAD_TO_MEMORY_RESULTS *pResults, PXOVERLAPPED pOverlapped )
{
	...
	TRACE("XStorageDownloadToMemory  (dwUserIndex = %d, wszServerPath = %s, dwBufferSize = %X, pbBuffer = %X, cbResults = %d, pResults = %X, pXOverlapped = %X)", dwUserIndex, wszServerPath, dwBufferSize, pbBuffer, cbResults, pResults, pOverlapped);
...
DWORD WINAPI XLiveProtectedVerifyFile(HANDLE hContentAccess, VOID * pvReserved, PCWSTR pszFilePath)
{
	...
	TRACE("XLiveProtectedVerifyFile  (hContentAccess = %X, pvReserved = %X, pszFilePath = %s", hContentAccess, pvReserved, pszFilePath);
...
DWORD WINAPI XLiveContentCreateAccessHandle (DWORD dwTitleId, PXCONTENT_DATA pContentInfo, DWORD dwLicenseInfoVersion, FakePBuffer *xebBuffer, DWORD dwOffset, HANDLE * phAccess, PXOVERLAPPED pOverlapped)
{
	...
	TRACE("XLiveContentCreateAccessHandle  (titleId = %d, contentInfo = %X, licenseInfo = %d, pBuffer = %X, offset = %d, handle = %X, overlapped = %X", dwTitleId, pContentInfo, dwLicenseInfoVersion, xebBuffer, dwOffset, phAccess, pOverlapped);
...
LONG WINAPI XLiveContentGetPath (DWORD dwUserIndex, PXCONTENT_DATA pContentData, wchar_t * pszPath, DWORD * pcchPath)
{
	...
	TRACE("XLiveContentGetPath  (dwUserIndex = %x, pXContentData = %x, pszPath = %x, pcchPath = %d)", dwUserIndex, pContentData, pszPath, *pcchPath);
	...
	TRACE("- ContentNum = %d, TitleId = %08X, ContentId = %02X%02X%02X%02X", pContentData->ContentNum, pContentData->TitleId, pContentData->ContentId[0], pContentData->ContentId[1], pContentData->ContentId[2], pContentData->ContentId[3]);
...
DWORD WINAPI XContentCreatePackage(DWORD dwUserIndex, PXCONTENT_DATA pContentData, WCHAR *pszPath, DWORD *pcchPath)
{
	...
	TRACE("XContentCreatePackage (?) (dwUserIndex = %x, pXContentData = %x, pszPath = %x, pcchPath = %d)", dwUserIndex, pContentData, pszPath, *pcchPath);
...
DWORD WINAPI XContentCreateEnumerator(DWORD MaxEnumerator, PDWORD a2, PDWORD pchBuffer, PHANDLE phEnum)
{
	...
	if(print < 15)
	{
		...
		TRACE("XContentCreateEnumerator  (MaxEnumerator = %X, a2 = %X, pchBuffer = %X, phEnum = %X", MaxEnumerator, a2, pchBuffer, phEnum);
	...
	if (print < 15)
	{
		...
		if (pchBuffer)
		{
			...
			TRACE("- pchBuffer = %X  (%d)", *pchBuffer, *pchBuffer / sizeof(XCONTENT_DATA));
...
DWORD WINAPI XContentRetrieveOffersByDate(DWORD dwUserIndex, DWORD dwOffserInfoVersion, SYSTEMTIME * pstStartDate, void * pOffserInfoArray, DWORD * pcOfferInfo, PXOVERLAPPED pOverlapped)
{
...
DWORD WINAPI XLivePBufferSetByteArray(FakePBuffer * pBuffer, DWORD offset, BYTE *source, DWORD size)
{
	...
	if(print < 35)
	{
		...
		TRACE("XLivePBufferSetByteArray  (pBuffer = %X, offset = %X, source = %X, size = %X)", pBuffer, offset, source, size);
...
DWORD WINAPI XLivePBufferGetByteArray(FakePBuffer * pBuffer, DWORD offset, BYTE * destination, DWORD size)
{
	...
	if(print < 35)
	{
		...
		TRACE("XLivePBufferGetByteArray  (pBuffer = %X, pBuffer->Id = %X, offset = %d, dest = %X, size = %d", pBuffer, pBuffer->id, offset, destination, size);
...
DWORD WINAPI XLivePBufferSetByte(FakePBuffer * pBuffer, DWORD offset, BYTE value)
{
	...
	if(print < 35)
	{
		...
		TRACE("XLivePBufferSetByte  (pBuffer = %X, offset = %X, value = %X)", pBuffer, offset, value);
...
DWORD WINAPI XLivePBufferGetByte(FakePBuffer * pBuffer, DWORD offset, BYTE * value)
{
	...
	if(print < 35)
	{
		...
		TRACE("XLivePBufferGetByte  (pBuffer = %X, offset = %X, value = %X)", pBuffer, offset, value);
...
DWORD WINAPI XLivePBufferGetDWORD(FakePBuffer * pBuffer, DWORD dwOffset, DWORD * pdwValue)
{
	...
	if(print < 35)
	{
		...
		TRACE("XLivePBufferGetDWORD  (pBuffer = %X, dwOffset = %X, pdwValue = %X)", pBuffer, dwOffset, pdwValue);
...
DWORD WINAPI XLivePBufferSetDWORD(FakePBuffer * pBuffer, DWORD dwOffset, DWORD dwValue)
{
	...
	if(print < 35)
	{
		...
		TRACE("XLivePBufferSetDWORD  (pBuffer = %X, dwOffset = %X, dwValue = %X)", pBuffer, dwOffset, dwValue);
...
HRESULT WINAPI XLiveProtectedCreateFile(HANDLE hContentAccess, void * pvReserved, PCWSTR pszFilePath, DWORD dwDesiredAccess, DWORD dwShareMode, SECURITY_ATTRIBUTES * pSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, PHANDLE phModule)
{
	
	TRACE("XLiveProtectedCreateFile  (hContentAccess = %X, pvReserved = %X, pszFilePath = %s, dwDesiredAccess = %d, dwShareMode = %d, dwSecurityAttributes = %X, dwCreationDisposition = %d, flagsAndAttributes = %d, phModule = %X)", hContentAccess, pvReserved, pszFilePath, dwDesiredAccess, dwShareMode, pSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, phModule);
	...
	*phModule = CreateFileW(dlcfile, dwDesiredAccess, dwShareMode, pSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, 0);
...
DWORD WINAPI XContentGetMarketplaceCounts( DWORD dwUserIndex, DWORD dwContentCategories, DWORD cbResults, XOFFERING_CONTENTAVAILABLE_RESULT *pResults, PXOVERLAPPED pOverlapped )
{
	...
	TRACE("XContentGetMarketplaceCounts  (dwUserIndex = %d, dwContentCategories = %X, cbResults = %X, pResults = %X, pOverlapped = %X)", dwUserIndex, dwContentCategories, cbResults, pResults, pOverlapped);
...
DWORD WINAPI XMarketplaceCreateOfferEnumerator(DWORD dwUserIndex, DWORD dwOfferType, DWORD dwContentCategories, DWORD cItem, PDWORD pcbBuffer, PHANDLE phEnum)
{
	...
	TRACE("XMarketplaceCreateOfferEnumerator  (dwUserIndex = %d, dwOfferType = %X, dwContentCategories = %X, cItem = %d, pchBuffer = %x, phEnum = %x", dwUserIndex, dwOfferType, dwContentCategories, cItem, pcbBuffer, phEnum);
...
DWORD WINAPI XMarketplaceCreateOfferEnumeratorByOffering(DWORD dwUserIndex, DWORD cItem, const ULONGLONG *pqwNumOffersIds, WORD cOfferIDs, PDWORD pcbBuffer, PHANDLE phEnum)
{
	...
	TRACE("XMarketplaceCreateOfferEnumeratorByOffering  (dwUserIndex = %d, cItem = %d, pqwNumOffersIds = %X, cOfferIDs = %d, pcbBuffer = %X, phEnum = %X)", dwUserIndex, cItem, pqwNumOffersIds, cOfferIDs, pcbBuffer, phEnum);
...
DWORD WINAPI XShowMessageBoxUI( DWORD dwUserIndex, LPCWSTR wszTitle, LPCWSTR wszText, DWORD cButtons, LPCWSTR *pwszButtons, DWORD dwFocusButton, DWORD dwFlags, MESSAGEBOX_RESULT *pResult, XOVERLAPPED *pOverlapped )
{
...
DWORD WINAPI XLiveLess_5237(DWORD a1)
{
	...
	TRACE("XLiveLess_5237  (*** checkme ***)  (a1 = %X)", a1);
...
DWORD WINAPI XLiveLess_5257(DWORD a1, DWORD a2, DWORD a3, DWORD dwData)
{
	...
	TRACE("XLiveLess_5257  (*** checkme ***) (a1 = %X, a2 = %X, a3 = %X, dwData = %X)", a1, a2, a3, dwData);
...
DWORD WINAPI XLiveLess_5288( DWORD a1, DWORD a2, DWORD a3, DWORD a4 )
{
	...
	TRACE("XLiveLess_5288  (*** checkme ***) (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
DWORD WINAPI XLiveLess_5289(DWORD a1, DWORD a2, DWORD a3)
{
	...
	TRACE("XLiveLess_5289  (*** checkme ***) (a1 = %X, a2 = %X, a3 = %X)", a1, a2, a3);
...
DWORD WINAPI XLiveLess_5296(DWORD a1)
{
	...
	TRACE("XLiveLess_5296  (*** checkme ***) (a1 = %X)", a1);
...
DWORD WINAPI XLiveLess_5299(DWORD a1)
{
	...
	TRACE("XLiveLess_5299  (*** checkme ***) (a1 = %X)", a1);
...
DWORD WINAPI XStorageUploadFromMemoryGetProgress(DWORD a1, DWORD a2, DWORD a3, DWORD a4)
{
	...
	TRACE("XStorageUploadFromMemoryGetProgress  (*** checkme ***) (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
DWORD WINAPI XStorageDownloadToMemoryGetProgress(DWORD a1, DWORD a2, DWORD a3, DWORD a4)
{
	...
	TRACE("XStorageDownloadToMemoryGetProgress  (*** checkme ***) (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
DWORD WINAPI XStorageDelete(DWORD a1, DWORD a2, DWORD a3)
{
	...
	TRACE("XStorageDelete  (*** checkme ***) (a1 = %X, a2 = %X, a3 = %X)", a1, a2, a3);
...
// 5362: MarketplaceDoesContentIdMatch
DWORD WINAPI MarketplaceDoesContentIdMatch(CHAR *a1, DWORD a2)
{
	
	TRACE("MarketplaceDoesContentIdMatch  (*** checkme ***) (a1 = %s, a2 = %X)", a1, a2);
...
DWORD WINAPI XLiveLess_5370(DWORD a1, DWORD a2, DWORD a3, DWORD a4)
{
	...
	TRACE("XLiveLess_5370  (*** checkme ***) (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
DWORD WINAPI XLiveLess_5371(DWORD a1, DWORD a2, DWORD a3, DWORD a4)
{
	...
	TRACE("XLiveLess_5371  (*** checkme ***) (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
DWORD WINAPI XLiveLess_5023(DWORD a1, DWORD a2)
{
	...
	TRACE("XLiveLess_5023  (a1 = %X, a2 = %X)", a1, a2);
...
DWORD WINAPI XLiveLess_5025(DWORD a1, DWORD a2, DWORD a3, DWORD a4)
{
	...
	TRACE("XLiveLess_5025  (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
DWORD WINAPI XLiveLess_5255(DWORD a1, DWORD a2, DWORD a3, DWORD a4, DWORD a5)
{
	...
	TRACE("XLiveLess_5255  (a1 = %X, a2 = %X, a3 = %X, a4 = %X, a5 = %X)", a1, a2, a3, a4, a5);
...
DWORD WINAPI XLiveLess_5341(DWORD a1, DWORD a2, DWORD a3)
{
	...
	TRACE("XLiveLess_5341  (a1 = %X, a2 = %X, a3 = %X)", a1, a2, a3);
...
DWORD WINAPI XLiveLess_5351(DWORD a1, DWORD a2, DWORD a3)
{
	...
	TRACE("XLiveLess_5351  (a1 = %X, a2 = %X, a3 = %X)", a1, a2, a3);
...
DWORD WINAPI XLiveLess_5357(DWORD a1, DWORD a2, DWORD a3, DWORD a4)
{
	...
	TRACE("XLiveLess_5357  (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
DWORD WINAPI XLiveLess_5358(DWORD a1, DWORD a2, DWORD a3)
{
	...
	TRACE("XLiveLess_5358  (a1 = %X, a2 = %X, a3 = %X)", a1, a2, a3);
...
HRESULT IXHV2ENGINE::StartLocalProcessingModes( VOID *pThis, DWORD dwUserIndex, VOID *processingModes, DWORD dwNumProcessingModes )
{
	
	TRACE("IXHV2Engine::StartLocalProcessingModes  (dwUserIndex = %X, processingModes = %X, dwNumProcessingModes = %d)", dwUserIndex, processingModes, dwNumProcessingModes);
	TRACE("- Voice chat on");
...
HRESULT IXHV2ENGINE::StopLocalProcessingModes( VOID *pThis, DWORD dwUserIndex, VOID *processingModes, DWORD dwNumProcessingModes )
{
	
	TRACE("IXHV2Engine::StopLocalProcessingModes  (dwUserIndex = %X, processingModes = %X, dwNumProcessingModes = %X)", dwUserIndex, processingModes, dwNumProcessingModes);
...
{
	TRACE("IXHV2Engine::StartRemoteProcessingModes  (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
	TRACE("- Voice chat on");
...
HRESULT IXHV2ENGINE::Dummy7( VOID *pThis, int a1, int a2, int a3, int a4 )
{
	...
	TRACE("IXHV2Engine::Dummy7  (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
HRESULT IXHV2ENGINE::RegisterLocalTalker( VOID *pThis, DWORD dwUserIndex )
{
	...
	TRACE("IXHV2Engine::RegisterLocalTalker  (dwUserIndex = %d)", dwUserIndex);
	TRACE("- user added");
...
HRESULT IXHV2ENGINE::UnregisterLocalTalker( VOID *pThis, DWORD dwUserIndex )
{
	...
	TRACE("IXHV2Engine::UnregisterLocalTalker  (dwUserIndex = %d)", dwUserIndex);
...
HRESULT IXHV2ENGINE::UnregisterRemoteTalker( VOID *pThis, int a1, int a2 )
{
	...
	TRACE("IXHV2Engine::UnregisterRemoteTalker  (a1 = %X, a2 = %X)", a1, a2);
	TRACE("- Voice stopped");
...
HRESULT IXHV2ENGINE::Dummy13( VOID *pThis, int a1, int a2 )
{
	...
	if(print < 15)
	{
		...
		TRACE("IXHV2Engine::Dummy13  (a1 = %X, a2 = %X)", a1, a2);
...
INT IXHV2ENGINE::Dummy14( VOID *pThis, int a1 )
{
	...
	if(print < 15)
	{
		...
		TRACE("IXHV2Engine::Dummy14  (a1 = %X)", a1);
...
INT IXHV2ENGINE::Dummy15( VOID *pThis, int a1 )
{
	...
	if(print < 15)
	{
		...
		TRACE("IXHV2Engine::Dummy15  (a1 = %X)", a1);
...
HRESULT IXHV2ENGINE::Dummy16( VOID *pThis, int a1, int a2 )
{
	...
	TRACE("IXHV2Engine::Dummy16  (a1 = %X, a2 = %X)", a1, a2);
...
HRESULT IXHV2ENGINE::GetLocalChatData(VOID *pThis, DWORD dwUserIndex, PBYTE pbData, PDWORD pdwSize, PDWORD pdwPackets)
{
	...
	if(print < 15)
	{
		...
		TRACE("IXHV2Engine::GetLocalChatData  (dwUserIndex = %X, pbData = %X, pdwSize = %X, pdwPackets = %X)", dwUserIndex, pbData, pdwSize, pdwPackets);
...
HRESULT IXHV2ENGINE::SetPlaybackPriority(VOID *pThis, int a1, int a2, int a3, int a4)
{
	...
	TRACE("IXHV2Engine::SetPlaybackPriority  (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
...
HRESULT IXHV2ENGINE::Dummy20( VOID *pThis, int a1, int a2, int a3, int a4 )
{
	...
	TRACE("IXHV2Engine::Dummy20  (a1 = %X, a2 = %X, a3 = %X, a4 = %X)", a1, a2, a3, a4);
